-- Lekhak AI PhonePe Payment Integration - COMPLETE REPLACEMENT
-- This replaces Stripe with PhonePe while preserving user data and structure
...
-- Keep users table but clean it up
...
DO $$
BEGIN
    RAISE NOTICE '✅ PhonePe-only database schema setup completed successfully!';
    RAISE NOTICE '🗑️ Stripe tables dropped and replaced with PhonePe integration';
    RAISE NOTICE '👥 User data preserved';
    RAISE NOTICE '📊 Tables created: users, subscription_plans, user_subscriptions, user_quotas, usage_logs, payment_transactions, phonepe_transactions, refunds, webhook_events, settlements, disputes, subscription_renewals';
    RAISE NOTICE '🔧 Functions updated: check_and_increment_quota (PhonePe version), reset_daily_quotas, reset_monthly_quotas';
    RAISE NOTICE '🛡️ Row Level Security enabled with service role policies';
    RAISE NOTICE '💰 PhonePe subscription plans inserted: Free (₹0), Pro (₹399), Unlimited (₹1599)';
    RAISE NOTICE '🚀 Database ready for PhonePe integration!';
END $$;